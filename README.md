...
REST API (`src/embeddings.py`) to use Hugging Face's sentence transformer to generate the vector data (embeddings: `all-MiniLM-L6-v2`). It is a basic HTTP service where a sentence is passed as a POST request and the vector data is returned, for example:
```
...
```
SELECT `QUERY`, JSON_RECORDS(GET_VECTOR_DATA('http://chatbot:9999/api/v1/embedding/sentence-transformer', `QUERY`)) as DATA FROM `chatbot-customer_actions` EMIT CHANGES;
```
